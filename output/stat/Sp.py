...
# 设置坐标轴范围和比例
ax.set_xlim(0.01, 2)
ax.set_ylim(1E-08, 100)
ax.set_xscale('log')
ax.set_yscale('log')

# 设置坐标轴标签
ax.set_xlabel(r'$r$', fontsize=46, fontfamily='Times New Roman', style='italic')
ax.set_ylabel(r'$S_p$', fontsize=46, fontfamily='Times New Roman')
...
# 定义线条和符号的颜色和样式
line_colors = ['#2c7bb6', '#8dd3c7', '#d73027']
line_thickness = [4, 1]
symbol_size = [10, 20]
...
for i, data in enumerate(data_sets):
    ...
    for j in range(2,5):
        ...
        up_DNS = 0.7782
        if i < 1:
            line_label = fr'$S_{{{j}}}$ WT'
        else:
            line_label = fr'$S_{{{j}}}$ DNS'
            y = y/up_DNS**j
            if j % 2 == 1:
                y = -y
        ...
        line, = ax.plot(x_valid, y_valid, color=line_colors[j-2], linewidth=line_thickness[i], label=line_label, marker='o',
...
# 调整标记大小
legend = ax.legend(new_handles, new_labels, prop=font, loc='lower center', bbox_to_anchor=(0.77, -0.08), frameon=False, markerscale=1,
                   handletextpad=0.15,  # 调整图例标记和文本之间的间距
                   labelspacing=-0.05)  # 调整图例项之间的垂直间距
# 添加文本注释
#ax.text(0.55, 0.77, r'$k^{{-5/3}}$', transform=ax.transAxes, fontsize=56, fontfamily='Times New Roman',color='black')
